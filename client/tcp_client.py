...
class TcpFileClient:
    ...
    def login(self, username: str, password: str) -> None:
        ...
        pass_b = password.encode("utf-8")  # 密码 UTF-8 编码
        payload = struct.pack("!H", len(user_b)) + user_b  # 用户名长度+用户名
        payload += struct.pack("!H", len(pass_b)) + pass_b  # 密码长度+密码
    ...
    def download_file(self, remote_name: str) -> None:
        ...
        save_path = os.path.join(DOWNLOAD_DIR, os.path.basename(filename))  # 本地保存路径

        received = 0  # 已接收字节计数
        with self._lock:  # 加锁接收文件流
            with open(save_path, "wb") as fp:  # 二进制写模式创建本地文件
                while received < file_size:  # 循环直到收满
                    to_read = min(CHUNK_SIZE, file_size - received)  # 末块大小
                    chunk = recv_exact(sock, to_read)  # 从 Socket recv 缓存读取一块
                    fp.write(chunk)  # 实时写入本地文件指针位置
